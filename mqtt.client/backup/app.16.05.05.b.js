  ...
  if (!server.proxies) server.proxies = {};
  if (!server.clients) server.clients = {}; // clients shuld exist in server.js of mosca

  process.on('SIGINT', function() {
      console.log('Received interrupt');
      for (c in server.clients) {
          server.clients[c].emit('close');
      }
      process.exit();
  });

  server.authenticate = function(client, username, password, callback) {
      console.log('client connection try to authenticate ...');
